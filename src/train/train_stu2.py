...
if __name__ == '__main__':
    ...
    with tf.Session() as sess:
        train_loader = Loader(train_input_file, delimiter, raw_size, processed_size, is_training, batch_size, num_prefetch,
                 num_threads, train_path_prefix, num_classes, shuffle=True, inference_only=False)
        ...
        eval_loader = Loader(eval_input_file, delimiter, raw_size, processed_size, is_training, eval_batch_size, eval_num_prefetch,
                 num_threads, eval_path_prefix, num_classes, shuffle=True, inference_only=False)
        ...
        try:
            for j in range(num_epochs):
                all_steps = int(all_train_data_size / batch_size)
                if(all_train_data_size % batch_size != 0):
                    all_steps += 1
                for now_step in range(1, all_steps + 1):
                    if(coord.should_stop()):
                        break
                    t_i, t_l = sess.run([train_image_batch, train_label_batch])
                    # print(i[0][0][0])
                    _, loss, acc = sess.run([end_points['optimizer'], end_points['loss'], end_points['accuracy']], feed_dict = {end_points['x']: t_i, end_points['y']: t_l})
                    print('run time: ' + str(time.time() - start_time) + '\tbatch: ' + str(j + 1)  + '\tstep: '  + str(now_step) + '\tdata size: ' + str(t_i.shape) + ', ' + str(t_l.shape) + '\tbatch loss: ' + str(loss) + '\tacc: ' + str(acc))
                    if(now_step % 100 == 0):
                        correct_times = 0
                        all_times = 0
