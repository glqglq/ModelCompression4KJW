...
def vgg_16(inputs,
  ...
  Returns:
    ...
    end_points: a dict of tensors with intermediate activations.
  """
  with tf.variable_scope(scope, 'vgg_16_stu', [inputs]) as sc:
    end_points_collection = sc.original_name_scope + '_end_points'
    # Collect outputs for conv2d, fully_connected and max_pool2d.
    with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
      ...
      # Use conv2d instead of fully_connected layers.
      net = slim.conv2d(net, 244, [7, 7], padding=fc_conv_padding, scope='fc6')
      #shape = int(np.prod(net.get_shape()[1:]))
      #net = slim.fully_connected(tf.reshape(net, [-1, shape]), 256, scope='fc6')
      net = slim.dropout(net, dropout_keep_prob, is_training=is_training,
                         scope='dropout6')
