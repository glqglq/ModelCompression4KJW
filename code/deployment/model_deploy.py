...
_deployment_params = {'num_clones': 2,
                      'clone_on_cpu': False,
                      'replica_id': 0,
                      'num_replicas': 1,
                      'num_ps_tasks': 0,
...
class DeploymentConfig(object):
  ...
  def __init__(self,
               num_clones=2,#每个单机部署多少个clone（即部署在多少个GPU）
               clone_on_cpu=False,
               replica_id=0,#整数，模型所部署的单机的索引，通常是0
               num_replicas=1,#使用多少个单机，通常为1，表示单机部署。
                              # 此时`worker_device`, `num_ps_tasks`和 `ps_device`这几个参数将被忽略。
               num_ps_tasks=0,#‘ps’作业(分布式作业)使用多少个单机，如果为0表示不使用单机
               worker_job_name='worker',#默认为“worker”
               ps_job_name='ps'):#默认为'ps'
