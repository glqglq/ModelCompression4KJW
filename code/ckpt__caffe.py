...
checkpoint_path = "/tmp/tfmodel_stu/model.ckpt-174839"
reader = pywrap_tensorflow.NewCheckpointReader(checkpoint_path)
var_to_shape_map = reader.get_variable_to_shape_map()

cf_prototxt = "/tmp/data/VGG_ILSVRC_16_stu_layers_deploy.prototxt"
cf_model = "/tmp/data/vgg_16_stu.caffemodel"
        ...
        try:
            if 'weights' in key_i:
                ...
                if (len(a) == 3):
                    ...
                    key_caffe = a[2]
                if (key_caffe == 'fc6'):
                    weights = tensor4d_transform(reader.get_tensor(key_i).reshape([7, 7, 512, 244])).reshape(
                        [[7, 7, 512, 244][3], -1])
                elif key_caffe == 'fc7':
